...
Enormous surface gravitational acceleration of $g\sim 10^14$ to $10^{15} \cmss$.
Strong gravitational field also bends the photon trajectories.\cite{PFC83}
First low-field models by pioneering work of Romani.\cite{Romani87}
...
\ee
Gravitational acceleration under general relativistic theory is then
\be
g = \frac{G M}{R^2} \frac{1}{\sqrt{1-u}} = \Ten{1.38}{14} \frac{1}{\sqrt{1-u}} \left( \frac{M}{\Msun} \right) \left( \frac{R}{10\km} \right)^2 \cmss
\ee
...
Because the thickness of the atmosphere is much smaller than the radius of the star, the atmosphere can be considered in plane-parallel approximation.
...
The standard approach in describing the atmosphere structure includes solving three main equations.
...
\be
S_{\nu} = (\sigma_{\nu} J_{\nu} + \alpha B_{\nu}) k_{\nu}^{-1}
\ee, that can be described using the mean spectral intensity 
\be
...
B_{\nu} = XX
\ee
is the thermal Planck function.
As an boundary conditions for the RTE we can use $I_{\nu} = 0$ for $\mu < 0$ at $y = 0$.

The atmospheres are also usually considered to be in radiative and hydrostatic equilibirum. 
First requirement can be formulated as 
\be
...
\ee
if the radiative pressure forces are neglected.

Finally, we need to supplement these equations with an equation connecting the pressure and density, the EoS.
