...
In thermodynamics, we speak of \emph{state variables} that describe a current state of the matter under given physical conditions. \blue{remove a, or s}\green{JN:done}.
...
In contrast to the ``normal'' matter where statistical moments \blue{remove a, or s}\green{JN:done} such as temperature can be used to describe a large ensemble of particles, the degenerate matter is dominated by quantum mechanical effects of single particles.
Because of the immense densities, a free particle in degenerate matter \blue{remove a}\green{JN:done} is actually bounded into a finite volume.
...
In the Earthly laboratories we can probe the matter somewhere close to $10^{14} \gcm$, after which the densities becomes too great for us to manipulate \blue{handle in reads weird}.\green{JN:done}%
...
Such condensed surfaces are, however, rare and usually the gaseous description is more than enough to give an accurate  description of the physics of the atmosphere \blue{enough to do what? accurate enough to xxx?}\green{JN:done}.\cite[for a review, see][]{ZP02, Potekhin14}
...
However, even a small accreted mass of $10^{-17}\Msun$, originating from the surrounding interstellar medium or a binary companion, \blue{I would add here: or a binary companion}\green{JN:done} is enough to cover the whole star, and hence a variety of elements are also expected in the neutron star atmospheres.
...
\ee
where $G$ is the gravitational constant, $c$ is the speed of light, and $M$ is the mass of the star \blue{is \Msun\ defined yet? The M there should be roman font, as its a unit, not a variable.}\green{JN:done}.
Hence, neutrons \blue{remove s}\green{JN:done} star has a compactness parameter in the range of $u \approx 1/5$ to $1/2$ resulting in considerable general relativistic corrections \blue{remove a or s}\green{JN:done}.
In comparison, the Sun has $u \approx \Ten{4.24}{-6}$.
Gravitational acceleration under general relativistic theory is \blue{add reference to a book or classical paper?}
\be
g = \frac{G M}{R^2} \frac{1}{\sqrt{1-u}} = \Ten{1.38}{14} \frac{1}{\sqrt{1-u}} \left( \frac{M}{\Msun} \right) \left( \frac{10\km}{R} \right)^2 \cmss.
\ee
Hence, a surface gravitational acceleration \blue{remove a or s}\green{JN:done} $g$ of $\sim 10^{14}$ to $\sim 10^{15}\cmss$ is expected for neutron stars.
By considering a barometric atmosphere \blue{what is that?} we can also estimate the scale height as
\be
...
From here, the typical scale height values of $\sim 1\cm$ to $\sim 10\cm$ are re-obtained \blue{remove re?} for atmospheres of $T=10^6$ and $10^7\Kelvin$.\cite{ZP02, Potekhin14}
...
\blue{I have never liked "basic equations", I think its a "Russian" style of starting Section 2; why not use: Radiative transport in the atmosphere, or something like that?} The standard approach in describing the atmosphere structure includes solving three main equations of radiative transfer, hydrostatic balance, and energy conservation.
...
\ee
where we have assumed that the radiation does not depend on the azimuthal $\phi$ angle (as is typical for atmosphere calculations) and introduced $\mu = \cos\theta$ \blue{use equiv?}.
...
\ee
\blue{power 4 missing!} where $\sigma_{\mathrm{SB}} = \Ten{5.67}{-5} \unitspace \g \unitspace \mathrm{s}^{-3} \unitspace \Kelvin^{-4}$ is the Stefan-Boltzmann constant \blue{I prefer erg/cm2/s/K4 as it shows that it relates temperature to the flux} and $T_{\mathrm{eff}}$ is the effective temperature of the atmosphere.
...
Mass for a hydrogen plasma is, on the other hand, mainly set by the protons, hence, opacity per mass \blue{opacity per mass seems an odd way to say this. Isnt it just opacity, or opacity per unit mass} is then
\be
...
\ee
For neutron stars close to $T \sim 10^7 \Kelvin$ this limit can be reached, after which the atmosphere is momentarily blown away \blue{blown away isnt the perfect phrase here as we dont have good evidence that it is always blown away}.
...
In general, the heat capacity is $C_P = C_P(T)$, but the main results hold, that observationally we should see a powerlaw decay of luminosity, if an excess energy is instantaneously injected or released into the deeper layers of the star. \blue{yleiskommenttina totean etta hyvin kirjoitettu ja selkee kappale, mutta ehka osittain jopa vahan turhan yksityiskohtainen; esim. yhtalon 1.19 johtaminen tolleen ei ehka ole tarpeen. }
...
Between the atmosphere and crust, a liquid ocean of ions also exists, but the interface is not very strict and the matter is smoothly evolving from an \blue{remove an, or use a; "an one" is not correct} one state to another.
The solidified crust is also typically divided into an outer and inner layers \blue{remove an, or s}, but the interface is again ambiguous. 
The pressure here is almost fully given by the degenerate electrons, and hence the matter is \blue{rephrase after this} familiar already from white dwarfs.
In the beginning, the electrons can be taken to be non-relativistic but after about $\rho \sim 10^{6} \gcm$ they turn into ultra-relativistic \blue{remove into, or add electrons} because of the increasing density.

By definition, the outer crust is a layer in the neutron star interiors \blue{remove s} where the plasma consists of electrons and nuclei, whereas the inner crust is characterized by an additional appearance of neutrons that start to drip out from the extremely neutron-rich nuclei.
The density this occurs is called the neutron drip density and is of around $\rho_{\mathrm{ND}} \sim \Ten{4}{11} \gcm$.
The outer crust, when defined to begin from the atmosphere at $\rho \sim 10^3 \gcm$ and continue to about $\rho_{\mathrm{ND}}$ is only about some hundred meters in thickness \blue{change "in thickness" to thick?}.
...
Additionally, the $g$ is the number of different quantum states a particle may have inside the cell, $E$ is the total particle energy, and $\mu$ is the chemical potential explained more carefully later on \blue{note that $\mu$ is already defined as $\mu = \cos \theta$}.
...
\ee
that describes the possible path of how a neutron $n$ will decay into a proton $p$, electron $e^{-}$, and electron \blue{ANTI!} neutrino $\bar{\nu_{e}}$.
...
When the temperature is decreased, the plasma will then, without a phase-transition \blue{why no phase transition?
}, become a strongly coupled Coulomb liquid corresponding to the neutron star ocean.
