...
\be
n_{\mathrm{f}} = \frac{8\pi}{h^3} \int_0^{p_{\mathrm{F}}} p^2 dp = \frac{8\pi}{3} \left( \frac{p_{\mathrm{F}}}{h} \right)^3.
\ee
...
\ee
In the weak-coupling limit ($\Gi \ll 1$) the Debye-H\"uckel results for the free energy are valid\cite{LL80}
\be
...
For a degenerate system it also makes sense to recall the Fermi quantities:
\be
...
= \Tr ( \gammar -1 ),
\ee
defining the momentum, energy, and temperature, respectively.
For the last expression, we have defined a typical temperature of
\be
...
\ee
and dimensionless Fermi momentum (in units of electron rest mass)
\be
