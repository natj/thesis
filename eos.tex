...
In practice, this means describing the behavior of the matter from \refedel{a} densities of $\osim 10^{-3} \gcm$ to $\osim 10^{15} \gcm$, an impressive $18$ orders of magnitude range starting from a hot and rarefied electron corona to an ultra-dense neutron liquid.

The structure of the star can be roughly divided into three distinct sections: atmosphere, crust, and core.
\refe{These sections are visualized in Fig.~\ref{fig:pizza}.}
...
Detailed microphysics of such matter is still unknown\refe{,} and this is reflected in a large uncertainty in the actual size of the star that is still \refe{poorly constrained.}
...
Additionally, the upper \refe{horizontal} axis shows the evolution of the radial coordinate computed for one particular EoS (SLy, see \sect{sect:core}) and neutron star configuration (mass of $1.4\Msun$).
...
Equation of state is a thermodynamical equation connecting these state\refedel{s} variables together.
...
Inside this small volume, the energy levels of the particle are restricted to take only a discrete set of values called quantum states, because of the underlying wave-nature of the quantum\refe{-}mechanical description.
Hence, a notion of temperature, for example, does not make much sense.

\refe{An} overview of the EoS for the full range of densities relevant to neutron stars is shown in \fig{fig:eos}.
...
In the laboratories on Earth\refe{,} we can probe the matter somewhere close to $10^{14} \gcm$, after which the densities become\refedel{s} too great for us to manipulate.%
...
The densest naturally occurring element found on top of planet Earth is osmium that has an \refe{atomic} density of ``just'' $\rho \approx \Ten{2.2}{1}\gcm$.
}
...
Usually\refe{,} it consists of a thin layer of plasma and ranges from a few millimeters to \refe{a} couple of centimeters in height, but in some cases if the emerging radiation field is strong enough it can momentarily expand the atmosphere up to several hundreds of meters.
...
In the atmospheres of normal stars\refe{,} the composition is a mixture of multiple elements.
...
\refe{The compactness} is also directly related to the strength of the gravitational redshift as
\be
1+z = (1-u)^{-1/2},
\ee
that describes how an energy of a photon changes when it has to climb up the gravitational potential well that is created by the central star, for example.
...
\refe{The} strong gravitational field also bends the photon trajectories.\cite[see e.g.,][]{PFC83}
...
In addition\refe{,} we need the source function 
\be
S_{\nu} = (\sigma_{\nu} J_{\nu} + \alpha B_{\nu}) \kappa_{\nu}^{-1},
\ee
...
The first requirement assumes that energy is transported by radiation only, i.e., we neglect\refe{,} for example\refe{,} conduction and convection.
...
Additionally, the $g$ is the number of different quantum states a particle may have inside the cell, $E$ is the total particle energy, and $\mu$ is the chemical potential \refe{that describes} the free energy content of the \refe{system} per particle. %explained more carefully later on.
...
Let us now focus on fermions only\refedel{,} and select the $+$-sign (Fermi-Dirac distribution) in Eq. \eqref{eq:dist}.
Fermions are characterized as spin-$\frac{1}{2}$ particles.
\refe{In accordance} with the quantum mechanics, there is room for only one fermion inside an elementary cell \eqref{eq:pcell}.
\refe{The} degeneracy parameter $g$ is then equal to $2$ because we can have particles of spin $-\frac{1}{2}$ and $+\frac{1}{2}$ in the same ``physical location'' ($\Delta x \Delta y \Delta z$), as they still have different ``momentum locations'' ($\Delta p_x \Delta p_y \Delta p_z$) in phase space because of the non-equal spins.

The chemical potential $\mu$ in Eq. \eq{eq:dist} can be expressed as
\be
\mu = \ef +  (E -  E_{\mathrm{k}}),
\ee
where $\ef$ is a reference energy level known as the Fermi energy.
\refe{This} reflects how the chemical potential is equal to the amount of energy that the can be liberated from the system, also known as chemical energy.
...
Because of the Pauli exclusion principle, no more than one fermion can exist\refedel{s} in the same quantum state so the levels are filled in order, and all the higher states will remain empty.
...
\ee
\refe{It is,} of course, also related to the Fermi energy simply as
\be
\ef = \frac{1}{2} \frac{ p_{\mathrm{F}}^2 }{ m },
\ee
\refe{that} we previously defined merely as a reference energy level.
\refe{However,} from this, the physical interpretation should also become clear as Fermi energy corresponds to the energy of the highest quantum state occupied in the system when temperature reaches $0$.
...
\refe{The} normal beta-decay mode for the neutrons, on the other hand, is 
\be
...
The physics behind this \refe{are} quite simple\refe{,} and we repeat the calculations here to introduce the reader to the topic.
...
In practice\refe{,} we use the so-called ion-sphere model to describe our Coulomb liquid of ions.
...
\Mu = \frac{\bra{Z}}{\bra{A}},
\ee
where $\bra{Z}$ is the mean charge number of the atomic nuclei (for \refedel{an}\refe{a} one-component plasma it is simply $Z$), and $\bra{A}$ is the average number of nucleons bound by one nucleus.
...
From here it is easy to see that only the Coulomb interaction term $P_{\mathrm{ii}}$, should give a measurable correction\refedel{s} to the pressure at low temperatures because for the conditions inside neutron stars $\Theta_{\mathrm{r}}^{(\mathrm{i})}$ is very small.
...
Hence, in the three-dimensional description the pressure is given as one\refe{-}third of what we would get when inserting Eqs. \eqref{eq:dist}, \eq{eq:pshell}, and \eqref{eq:vstand} into \eqref{eq:vpress}:
\be
...
In the non-relativistic and ultra-relativistic regimes\refe{,} we can apply the asymptotic limits of Eq.\eq{eq:asymplims}.
...
Hence, normal atoms can no longer exists, and the matter becomes a\refedel{n} uniform plasma of neutrons, protons, and electrons.
...
Inner core of a neutron star can be many kilometers thick\refe{,} and the central densities of the most massive stars can go up to $\rho \approx (10-15) \times \nsat \approx (2.8 - 4.2) \times 10^{15} \gcm$.
...
\begin{itemize}
    ...
    \item normal nuclear matter \refedel{spiced up}\refe{mixed} with hyperons ($npe\mu + H$),
    \item normal nuclear matter together with more exotic particles like pion and kaon condensates ($npe\mu + \pi + K$), and
    \item matter consisting of (or normal matter \refedel{spiced up}\refe{mixed} with) quarks ($q$ or $npe\mu + q$).
\end{itemize}
...
\begin{figure}[t!]
\centering
\includegraphics[width=11.5cm]{figs/tov/core_eos_SLy.pdf}
\includegraphics[width=11.5cm]{figs/tov/core_gamma_SLy.pdf}
...
These 11 equations of states end up giving quite \refe{a} good overview of different models present in the literature.
...
This is an important feature of the equations as it can be used, for example, to rule out certain equations of states, given that we have measured some (large) neutron star mass to exist\refedel{s} in Nature.
...
From these arguments, it would seem that neutron stars can not exist\refedel{s} because the maximum mass is less than that of the white dwarfs.
