...
\chapterimage[width=12.8cm]{wordcloud/chap4b.png}
...
In this thesis, we will focus on extracting information from neutron stars using astrophysical phenomena called X-ray bursts.
They are produced by an unstable thermonuclear runaways in the surface layers of the star and can be seen as end-results of accretion.
In theory, using the X-ray bursts to probe the neutron star interiors is a robust way to study the matter inside the star as we can theoretically model the characteristics of the emerging radiation and these models can be applied to describe the data that we see. 
...
In this final chapter, we will lay out the basics of how observing the burst cooling it is possible to set constraints on the size of the emitting area, and in the end, on the radius of the neutron star.
...
Accretion can be a powerful energy source but this is not the end of the material that slowly spirals down into the star.
...
In the end, the accreted matter is fused together into heavier elements, and at the same time, energy is released into the neutron star envelope.
...
\ee
where $B_E(T_{\mathrm{ obs}})$ is a blackbody function with a measured temperature $T_{\mathrm{obs}}$ and normalization $K_{\mathrm{obs}}$, together with $E$ which is the photon energy that we observe at.
...
\begin{figure}[t!]
\centering
\includegraphics[width=13.0cm]{figs/constraints/bursts.pdf}
...
The red ribbon shows the blackbody temperature (inner right-hand vertical axis) in keV. 
...
The local detailed model spectra are well approximated by a so-called diluted blackbody model given as
\be
...
Computing hot neutron star atmosphere models for the bursters is of paramount importance for obtaining the mass and radius measurements from the burst observations.
...
New atmosphere model fits to these bursts indicated that sometimes the photosphere can consist entirely of metals, i.e., nuclear burning ashes.
