...
Another observable, we can try and estimate is the magnetic field.\mnote{$B$-field}
...
When considering magnetic field, it is the magnetic flux through the surface that conserves, hence we expect the field to scale as a cube of the radius.
Using the same compression ratio of \Ten{7}{4} for the radii, we then obtain $B_{\mathrm{NS}} \approx 1 \times (\Ten{7}{4})^3 \Gauss \sim \Ten{3}{14} \Gauss$.
...
As a final task, let us try and estimate the final energetics of different phenomena of what we can observe from neutron stars.
\red{Luminosity}
