...
This is a process where matter is transferred from \refe{a one star} to another because of the gravitational forces.
...
Accretion is an astrophysical process that has its roots in the gravity. \refedel{potential energy.}
...
Such a setup leads to a mass-transfer quite naturally as the heavier neutron star will just rip out the outer layers of its poor companion and starts slowly \refe{consuming} it. \refedel{until nothing is left.}
...
From observations\refe{,} we also know that the disks alternates between three states called, quiescent, hard and soft state.\cite{Mitsuda89, HvdK89, GD02, MC03, MDF14, DGK07}
...
\refe{In the final quiescent}, or ``off'' state, the observed flux typically drops below the detection limit and we do not see anything.
...
\refe{In the quiescent} state, the flow of matter might then come to a halt completely.
...
The \refe{spin} frequency of this disk edge can be taken to be Keplerian, $\Omega_{\mathrm{K}}(R)/2\pi \sim 1500 \unitspace\mathrm{Hz}$.
...
\refe{In the end,} the accreted matter is fused together into heavier elements, and at the same time, energy is released into the neutron star envelope.
...
All in all, the energy production of bursts appears to be a complex mechanism\refedel{s} that we do not fully understand yet.\cite{Kuuttila17, intZand17}
